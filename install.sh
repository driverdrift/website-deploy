...
if [ "$EUID" -ne 0 ]; then
	...
	echo
  	if [[ "${1-}" =~ ^([Yy][Ee][Ss]|[Yy])$ ]]; then
		...
		echo 'sudo bash -c "$(wget -qO- https://raw.githubusercontent.com/botshell/website-deploy/main/install.sh)"'
	fi
	exit 1
fi

REPO_URL="https://github.com/botshell/website-deploy/archive/main.tar.gz"
